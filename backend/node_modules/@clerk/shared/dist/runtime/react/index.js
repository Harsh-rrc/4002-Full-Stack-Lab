...
* @internal
*/
function readOAuthConsentFromSearch(search) {
	const sp = new URLSearchParams(search);
	const oauthClientId = sp.get("client_id") ?? "";
	const scopeValue = sp.get("scope");
	if (scopeValue === null) return { oauthClientId };
	return {
		oauthClientId,
		scope: scopeValue
	};
...
function useOAuthConsent(params = {}) {
	...
	const fromUrl = (0, react.useMemo)(() => {
		if (typeof window === "undefined" || !window.location) return { oauthClientId: "" };
		return readOAuthConsentFromSearch(window.location.search);
	}, []);
	const oauthClientId = (oauthClientIdParam !== void 0 ? oauthClientIdParam : fromUrl.oauthClientId).trim();
	const scope = scopeParam !== void 0 ? scopeParam : fromUrl.scope;
